...
import com.arindamv1.Sharesio.fragment.HomeFragment;
    ...
    private void applyAwaitingDrawerAction()
    {
        if (mChosenMenuItemId == 0) {
         ...
         //   finish();
            ...
            Uri uri = Uri.parse("market://search?q=AD2Apps" + "PA Production"); //Developer AC Name
            Intent goToMarket = new Intent(Intent.ACTION_VIEW, uri);
            try {
                startActivity(goToMarket);
            } catch (ActivityNotFoundException e) {
                startActivity(new Intent(Intent.ACTION_VIEW,
                        Uri.parse("http://play.google.com/store/search?q=AD2Apps" + ""))); //Developer AC Name
            }
        }
